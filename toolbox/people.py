...
# the new one
# from toolbox.api_requests import get_github_api_host, retrieve_data
import re
from settings import RE_NAME


def retrieve_members(api, args, get_2fa_disabled=False):
    ...
    template = 'https://{0}/orgs/{1}/members'.format(
        api.get_github_api_host(),
        args.user)

    return api.retrieve_data(
        args, template,
        single_request=single_request, query_args=query_args)
...
#         # get those who don't enable 2FA in our Org
#         print(
#             'retrieve outside_contributors who DID ',
#             'NOT enable 2fa from an organization')
...
#     return [
#         retrieve_user_details(args,github_acct_name,keys)
#         for github_acct_name
#         in namelist]
...
#     '''
#     parsed_results = [item['login']
#         for item
#         in input_users
#         if item['login'] not in whitelist]
...
#         details = retrieve_user_details(
#             args=args,
#             github_acct_name=individual_login, keys=keys)
#         if not details['name']:
#             print('NAMELESS!!! ')
...
#             print(
#                 'expanding original list with ',
#                 '2fa-disabled-only user: {}'.format(two_fa_disabled_user))
#             details = retrieve_user_details(args, two_fa_disabled_user, keys)
#             details['reject_reason']='2fa'
#             list_nameless.append(details)
#         else:
#             print(
#                 'merging an user who did not do ',
#                 '2fa && have no valid name.. do nothing')
